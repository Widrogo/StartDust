...
//Change x and y location of each asteroid
d3.timer(function() {
		//Move the asteroid - DO NOT USE TRANSITION
...
function removeEvents() {
	...
	d3.selectAll('.asteroid').on('mouseover', null).on('mouseout', null);
...
//Highlight the chosen asteroid and its orbit
function highlight(asteroid, delayTime){
...
//Function to bring opacity back of all asteroids
...
//Dim all other asteroids (and orbits)
function dimOne(asteroid, delayTime) {

	if(typeof(delayTime)==='undefined') delayTime = 0;
	var time = 500;

	//Dim all other asteroids
	svg.selectAll(".asteroid")
		.filter(function(d, i) {return i == asteroid;})
		.transition().delay(700 * delayTime).duration(time)
